...
def feature_visualization_m(x, m_f, save_dir=Path("runs/detect/exp")):
    ...
    if m_f == 9:
        stage = 'stage5'
    # x_rgb, x_ir = x
    # x_rgb = x_rgb[0].permute(1, 2, 0).cpu().numpy() * 255
    # x_ir = x_ir[0].permute(1, 2, 0).cpu().numpy() * 255
    # y_rgb = np.max(x_rgb, axis=2)
    # y_ir = np.max(x_ir, axis=2)
    # cv2.imwrite(save_dir+'_'+stage+'_rgb.png', y_rgb)
    ...
    #     blocks[1] += x[0][:, i, :, :].cpu()
    # _, ax = plt.subplots(1, 2, tight_layout=True)  # 8 rows x n/8 cols
    # ax = ax.ravel()
...
class BaseModel_m(nn.Module):
    ...
    def _predict_once(self, x_rgb, x_ir, profile=False, visualize=False, embed=None):
        """
        Perform a forward pass through the network.
        ...
        # pearson = False
        # if pearson:
        #     prefix = 'adalora_sym_r9-6'
        #     f_stage1 = open(f'{prefix}_stage1.csv', 'a', encoding='utf-8')
        #     f_stage2 = open(f'{prefix}_stage2.csv', 'a', encoding='utf-8')
        #     f_stage3 = open(f'{prefix}_stage3.csv', 'a', encoding='utf-8')
        #     f_stage4 = open(f'{prefix}_stage4.csv', 'a', encoding='utf-8')
        #     f_stage5 = open(f'{prefix}_stage5.csv', 'a', encoding='utf-8')
        #     f1 = csv.writer(f_stage1)
        #     f2 = csv.writer(f_stage2)
        #     f3 = csv.writer(f_stage3)
        #     f4 = csv.writer(f_stage4)
        #     f5 = csv.writer(f_stage5)
        #     p1 = 1 # 20
        #     p2 = 3 # 20
        #     p3 = 10 # 20
        #     p4 = 11 # 21
        ...
        # visualize = True
        # save_dir='/home/data3/zys/results/DroneVehicle/feature_map/'

        # f_cnt = open('count.csv', 'r', encoding='utf-8')
        # fcnt = csv.reader(f_cnt)
        # index = int(next(fcnt)[0])
        # f_cnt.close()
        # save_count = index + 1
        ...
        # if visualize and x_rgb.shape[-1] != 32:
        #     input_rgb = x_rgb[0][:, 110:594, 110:721].permute(1, 2, 0).cpu().numpy() * 255
        ...
        for m in self.model:
            ...
            #             x0 = x[0][i].view(-1).cpu().numpy()
            #             x1 = x[1][i].view(-1).cpu().numpy()
            #             p = calculate_pearson(x0, x1)
            #             if m.i == p1:
            #                 f1.writerow([str(p)])
            #             if m.i == p2:
            #                 f2.writerow([str(p)])
    ...
    def _predict_augment(self, x_rgb, x_ir):
        ...
        return self._predict_once(x_rgb, x_ir)
...
class SegmentationModel_m(DetectionModel_m):
...
class PoseModel_m(DetectionModel_m):
    """YOLOv8 pose model."""
...
class ClassificationModel_m(BaseModel_m):
...
class RTDETRDetectionModel_m(DetectionModel_m):
    """
    ...
    def loss(self, batch, preds=None):
        ...
        targets = {
            "cls": batch["cls"].to(img_rgb.device, dtype=torch.long).view(-1),
            "bboxes": batch["bboxes"].to(device=img_rgb.device),
            "batch_idx": batch_idx.to(img_rgb.device, dtype=torch.long).view(-1),
            "gt_groups": gt_groups,
        }

        preds = self.predict(img_rgb, img_ir, batch=targets) if preds is None else preds
        ...
        return sum(loss.values()), torch.as_tensor(
            [loss[k].detach() for k in ["loss_giou", "loss_class", "loss_bbox"]], device=img_rgb.device
        )

    def predict(self, x_rgb, x_ir, profile=False, visualize=False, batch=None, augment=False, embed=None):
        """
...
class WorldModel_m(DetectionModel_m):
    """YOLOv8 World Model."""
