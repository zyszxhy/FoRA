...
class AutoBackend_m(nn.Module):
    ...
    def forward(self, im_rgb, im_ir, augment=False, visualize=False, embed=None):
        """
        ...
        if self.pt or self.nn_module:  # PyTorch
            y = self.model(im_rgb, im_ir, augment=augment, visualize=visualize, embed=embed)
            ...
            y = self.session.run(self.output_names, {self.session.get_inputs()[0].name: im_rgb, \
                                                     self.session.get_inputs()[0].name: im_ir})
        elif self.xml:  # OpenVINO
            im_rgb = im_rgb.cpu().numpy()  # FP32
            im_ir = im_ir.cpu().numpy()  # FP32
            y = list(self.ov_compiled_model(im_rgb, im_ir).values())
        elif self.engine:  # TensorRT
            if self.dynamic and im_rgb.shape != self.bindings["images"].shape:
                i = self.model.get_binding_index("images")
                self.context.set_binding_shape(i, im_rgb.shape)  # reshape if dynamic
                self.bindings["images"] = self.bindings["images"]._replace(shape=im_rgb.shape)
            ...
            self.binding_addrs["images"] = int(im_rgb.data_ptr())
            self.context.execute_v2(list(self.binding_addrs.values()))
            y = [self.bindings[x].data for x in sorted(self.output_names)]
        elif self.coreml:  # CoreML
            im_rgb = im_rgb[0].cpu().numpy()
            im_pil_rgb = Image.fromarray((im_rgb * 255).astype("uint8"))
            im_ir = im_ir[0].cpu().numpy()
            im_pil_ir = Image.fromarray((im_ir * 255).astype("uint8"))
            # im = im.resize((192, 320), Image.BILINEAR)
            y = self.model.predict({"image_rgb": im_pil_rgb, "image_ir": im_pil_ir})  # coordinates are xywh normalized
            if "confidence" in y:
                raise TypeError(
            ...
            if self.saved_model:  # SavedModel
                ...
                y = self.frozen_func(x=self.tf.constant(im_rgb, im_ir))
                ...
                if integer:
                    ...
                    im_rgb = (im_rgb / scale + zero_point).astype(details["dtype"])  # de-scale
                    im_ir = (im_ir / scale + zero_point).astype(details["dtype"])  # de-scale
                self.interpreter.set_tensor(details["index"], im_rgb)
                self.interpreter.set_tensor(details["index"], im_ir)
