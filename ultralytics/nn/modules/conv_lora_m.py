...
class Conv2d_m(nn.Module):
    def __init__(self, c1, c2, k, r, lora_alpha, lora_dropout, \
        ...
        if r > 0:
            self.lora_A_rgb = nn.Parameter(self.conv.weight.new_zeros((r * k, c1 * k)))
            self.lora_B_rgb = nn.Parameter(self.conv.weight.new_zeros((c2//self.conv.groups*k, r * k)))
            self.lora_A_ir = nn.Parameter(self.conv.weight.new_zeros((r * k, c1 * k)))
            self.lora_B_ir = nn.Parameter(self.conv.weight.new_zeros((c2//self.conv.groups*k, r * k)))
    ...
    def reset_parameters(self):
        ...
        if hasattr(self, 'lora_A_rgb'):
            ...
            nn.init.zeros_(self.lora_B_rgb)
            nn.init.kaiming_uniform_(self.lora_A_ir, a=math.sqrt(5))
            nn.init.zeros_(self.lora_B_ir)

    def forward(self, x):
        x_rgb = self.conv._conv_forward(
                x[0],
                self.conv.weight + (self.lora_B_rgb @ self.lora_A_rgb).view(self.conv.weight.shape) * self.scaling,
                self.conv.bias
            )
        x_ir = self.conv._conv_forward(
                x[1],
                self.conv.weight + (self.lora_B_ir @ self.lora_A_ir).view(self.conv.weight.shape) * self.scaling,
                self.conv.bias
            )
        return (x_rgb, x_ir)
...
class Conv_lora_m(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, r=0, lora_alpha=1, lora_dropout=0., p=None, g=1, d=1, act=True):
        ...
        super().__init__()
        self.conv = Conv2d_m(c1, c2, k, r, lora_alpha, lora_dropout, \
...
class C2f_lora_m(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, r=0, lora_alpha=1, lora_dropout=0., g=1, e=0.5):
        ...
        self.cv1 = Conv_lora_m(c1, 2 * self.c, 1, 1, r, lora_alpha, lora_dropout)
        self.cv2 = Conv_lora_m((2 + n) * self.c, c2, 1, 1, r, lora_alpha, lora_dropout)  # optional act=FReLU(c2)
        self.m = nn.ModuleList(Bottleneck_lora_m(self.c, self.c, shortcut, g, k=(3, 3), e=1.0, \
...
class Bottleneck_lora_m(nn.Module):
    """Standard bottleneck."""
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5, r=0, lora_alpha=1, lora_dropout=0.):
        ...
        self.cv1 = Conv_lora_m(c1, c_, k[0], 1, r, lora_alpha, lora_dropout)
        self.cv2 = Conv_lora_m(c_, c2, k[1], 1, r, lora_alpha, lora_dropout, g=g)
...
class SPPF_lora_m(nn.Module):
    ...
    def __init__(self, c1, c2, k=5, r=0, lora_alpha=1, lora_dropout=0.):
        ...
        self.cv1 = Conv_lora_m(c1, c_, 1, 1, r, lora_alpha, lora_dropout)
        self.cv2 = Conv_lora_m(c_ * 4, c2, 1, 1, r, lora_alpha, lora_dropout)
