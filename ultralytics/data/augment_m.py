...
class Mosaic_m(BaseMixTransform_m):
    """
    Mosaic augmentation.
    ...
    def get_indexes(self, buffer=True):
        ...
        if buffer:  # select images from buffer
            return random.choices(list(self.dataset.buffer_rgb), k=self.n - 1)
    ...
    def _mosaic3(self, labels):
        ...
        for i in range(3):
            ...
            # Load image
            img_rgb = labels_patch["img_rgb"]
            img_ir = labels_patch["img_ir"]
            h, w = labels_patch.pop("resized_shape")

            # Place img in img3
            if i == 0:  # center
                img3_rgb = np.full((s * 3, s * 3, img_rgb.shape[2]), 114, dtype=np.uint8)  # base image with 3 tiles
                img3_ir = np.full((s * 3, s * 3, img_ir.shape[2]), 114, dtype=np.uint8)  # base image with 3 tiles
                h0, w0 = h, w
            ...
            img3_rgb[y1:y2, x1:x2] = img_rgb[y1 - padh :, x1 - padw :]  # img3[ymin:ymax, xmin:xmax]
            img3_ir[y1:y2, x1:x2] = img_ir[y1 - padh :, x1 - padw :]  # img3[ymin:ymax, xmin:xmax]
        ...
        final_labels["img_rgb"] = img3_rgb[-self.border[0] : self.border[0], -self.border[1] : self.border[1]]
        final_labels["img_ir"] = img3_ir[-self.border[0] : self.border[0], -self.border[1] : self.border[1]]
        return final_labels
    ...
    def _mosaic4(self, labels):
        ...
        for i in range(4):
            ...
            mosaic_labels.append(labels_patch)
        final_labels = self._cat_labels(mosaic_labels)
        final_labels["img_rgb"] = img4_rgb
        final_labels["img_ir"] = img4_ir
        return final_labels
    ...
    def _mosaic9(self, labels):
        ...
        for i in range(9):
            ...
            # Place img in img9
            if i == 0:  # center
                img9_rgb = np.full((s * 3, s * 3, img_rgb.shape[2]), 114, dtype=np.uint8)  # base image with 4 tiles
                img9_ir = np.full((s * 3, s * 3, img_ir.shape[2]), 114, dtype=np.uint8)  # base image with 4 tiles
                h0, w0 = h, w
                c = s, s, s + w, s + h  # xmin, ymin, xmax, ymax (base) coordinates
            elif i == 1:  # top
                c = s, s - h, s + w, s
            ...
            # Image
            img9_rgb[y1:y2, x1:x2] = img_rgb[y1 - padh :, x1 - padw :]  # img9[ymin:ymax, xmin:xmax]
            img9_ir[y1:y2, x1:x2] = img_ir[y1 - padh :, x1 - padw :]  # img9[ymin:ymax, xmin:xmax]
            hp, wp = h, w  # height, width previous for next iteration
        ...
        final_labels["img_rgb"] = img9_rgb[-self.border[0] : self.border[0], -self.border[1] : self.border[1]]
        final_labels["img_ir"] = img9_ir[-self.border[0] : self.border[0], -self.border[1] : self.border[1]]
        return final_labels

    @staticmethod
    def _update_labels(labels, padw, padh):
        """Update labels."""
        nh, nw = labels["img_rgb"].shape[:2]
    ...
    def _cat_labels(self, mosaic_labels):
        ...
        # Final labels
        final_labels = {
            "im_file_rgb": mosaic_labels[0]["im_file_rgb"],
            "im_file_ir": mosaic_labels[0]["im_file_ir"],
...
class MixUp_m(BaseMixTransform_m):
    ...
    def _mix_transform(self, labels):
        ...
        labels["img_rgb"] = (labels["img_rgb"] * r + labels2["img_rgb"] * (1 - r)).astype(np.uint8)
        labels["img_ir"] = (labels["img_ir"] * r + labels2["img_ir"] * (1 - r)).astype(np.uint8)
        labels["instances"] = Instances.concatenate([labels["instances"], labels2["instances"]], axis=0)
        labels["cls"] = np.concatenate([labels["cls"], labels2["cls"]], 0)
        return labels


class RandomPerspective_m:
    """
    ...
    def affine_transform(self, img_rgb, img_ir, border):
        """
        ...
        C[0, 2] = -img_rgb.shape[1] / 2  # x translation (pixels)
        C[1, 2] = -img_rgb.shape[0] / 2  # y translation (pixels)
        ...
        if (border[0] != 0) or (border[1] != 0) or (M != np.eye(3)).any():  # image changed
            if self.perspective:
                img_rgb = cv2.warpPerspective(img_rgb, M, dsize=self.size, borderValue=(114, 114, 114))
                img_ir = cv2.warpPerspective(img_ir, M, dsize=self.size, borderValue=(114, 114, 114))
            else:  # affine
                img_rgb = cv2.warpAffine(img_rgb, M[:2], dsize=self.size, borderValue=(114, 114, 114))
                img_ir = cv2.warpAffine(img_ir, M[:2], dsize=self.size, borderValue=(114, 114, 114))
        return img_rgb, img_ir, M, s
    ...
    def __call__(self, labels):
        ...
        img_rgb = labels["img_rgb"]
        img_ir = labels["img_ir"]
        cls = labels["cls"]
        instances = labels.pop("instances")
        # Make sure the coord formats are right
        instances.convert_bbox(format="xyxy")
        instances.denormalize(*img_rgb.shape[:2][::-1])

        border = labels.pop("mosaic_border", self.border)
        self.size = img_rgb.shape[1] + border[1] * 2, img_rgb.shape[0] + border[0] * 2  # w, h
        # M is affine matrix
        # Scale for func:`box_candidates`
        img_rgb, img_ir, M, scale = self.affine_transform(img_rgb, img_ir, border)
        ...
        labels["cls"] = cls[i]
        labels["img_rgb"] = img_rgb
        labels["img_ir"] = img_ir
        labels["resized_shape"] = img_rgb.shape[:2]
        return labels
...
class RandomHSV_m:
    ...
    def __call__(self, labels):
        ...
        if self.hgain or self.sgain or self.vgain:
            ...
            hue_rgb, sat_rgb, val_rgb = cv2.split(cv2.cvtColor(img_rgb, cv2.COLOR_BGR2HSV))
            hue_ir, sat_ir, val_ir = cv2.split(cv2.cvtColor(img_ir, cv2.COLOR_BGR2HSV))
            ...
            im_hsv_rgb = cv2.merge((cv2.LUT(hue_rgb, lut_hue), cv2.LUT(sat_rgb, lut_sat), cv2.LUT(val_rgb, lut_val)))
            im_hsv_ir = cv2.merge((cv2.LUT(hue_ir, lut_hue), cv2.LUT(sat_ir, lut_sat), cv2.LUT(val_ir, lut_val)))
            cv2.cvtColor(im_hsv_rgb, cv2.COLOR_HSV2BGR, dst=img_rgb)  # no return needed
            cv2.cvtColor(im_hsv_ir, cv2.COLOR_HSV2BGR, dst=img_ir)  # no return needed
...
class RandomFlip_m:
    ...
    def __call__(self, labels):
        ...
        labels["img_rgb"] = np.ascontiguousarray(img_rgb)
        labels["img_ir"] = np.ascontiguousarray(img_ir)
...
class LetterBox_m:
    ...
    def __call__(self, labels=None, image_rgb=None, image_ir=None):
        """Return updated labels and image with added border."""
        if labels is None:
            labels = {}
        img_rgb = labels.get("img_rgb") if image_rgb is None else image_rgb
        img_ir = labels.get("img_ir") if image_ir is None else image_ir
        ...
        img_rgb = cv2.copyMakeBorder(
            img_rgb, top, bottom, left, right, cv2.BORDER_CONSTANT, value=(114, 114, 114)
        )  # add border
        img_ir = cv2.copyMakeBorder(
            img_ir, top, bottom, left, right, cv2.BORDER_CONSTANT, value=(114, 114, 114)
        )  # add border
    ...
    def _update_labels(self, labels, ratio, padw, padh):
        """Update labels."""
        labels["instances"].convert_bbox(format="xyxy")
        labels["instances"].denormalize(*labels["img_rgb"].shape[:2][::-1])
...
class CopyPaste_m:
    ...
    def __call__(self, labels):
        ...
        if self.p and len(instances.segments):
            n = len(instances)
            _, w, _ = im_rgb.shape  # height, width, channels
            ...
            im_rgb[i] = result_rgb[i]
            im_ir[i] = result_ir[i]

        labels["img_rgb"] = im_rgb
        labels["img_ir"] = im_ir
        labels["cls"] = cls
        labels["instances"] = instances
        return labels
...
class Format_m:
    ...
    def __call__(self, labels):
        ...
        img_rgb = labels.pop("img_rgb")
        img_ir = labels.pop("img_ir")
        h, w = img_rgb.shape[:2]
        cls = labels.pop("cls")
        instances = labels.pop("instances")
        ...
        if self.return_mask:
            if nl:
                ...
                masks = torch.zeros(
                    1 if self.mask_overlap else nl, img_rgb.shape[0] // self.mask_ratio, img_rgb.shape[1] // self.mask_ratio
                )
            labels["masks"] = masks
        if self.normalize:
            instances.normalize(w, h)
        labels["img_rgb"] = self._format_img(img_rgb)
        labels["img_ir"] = self._format_img(img_ir)
...
def v8_transforms_m(dataset, imgsz, hyp, stretch=False):
    """Convert images to a size suitable for YOLOv8 training."""
    pre_transform = Compose(
        [
            Mosaic_m(dataset, imgsz=imgsz, p=hyp.mosaic),
            CopyPaste_m(p=hyp.copy_paste),
            RandomPerspective_m(
                degrees=hyp.degrees,
                ...
                pre_transform=None if stretch else LetterBox_m(new_shape=(imgsz, imgsz)),
            ),
        ]
    )
    ...
    return Compose(
        [
            pre_transform,
            MixUp_m(dataset, pre_transform=pre_transform, p=hyp.mixup),
            # Albumentations(p=1.0),
            RandomHSV_m(hgain=hyp.hsv_h, sgain=hyp.hsv_s, vgain=hyp.hsv_v),
            RandomFlip_m(direction="vertical", p=hyp.flipud),
            RandomFlip_m(direction="horizontal", p=hyp.fliplr, flip_idx=flip_idx),
        ]
    )  # transforms
