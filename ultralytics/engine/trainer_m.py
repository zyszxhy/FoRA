    ...
    def mask_to_target_rank(self, model, curr_rank): 
        ...
        for n,p in model.named_parameters(): 
            if "lora_A_rgb" in n: 
                ...
                name_mat = n.replace("lora_A_rgb", "%s")
            ...
            if "lora_A_ir" in n: 
                ...
                ipt_score = self.calculate_score(n, metric="ipt")
                comb_ipt = torch.mean(ipt_score, dim=1, keepdim=True)
                name_mat = n.replace("lora_A_ir", "%s")
                if name_mat not in combine_dict: 
                    combine_dict[name_mat] = [comb_ipt]
                else:
                    combine_dict[name_mat].append(comb_ipt)
            if "lora_B_rgb" in n: 
                ...
                name_mat = n.replace("lora_B_rgb", "%s")
                if name_mat not in combine_dict: 
                    combine_dict[name_mat] = [comb_ipt]
                else:
                    combine_dict[name_mat].append(comb_ipt)
            if "lora_B_ir" in n: 
                ...
                ipt_score = self.calculate_score(n, metric="ipt")
                comb_ipt = torch.mean(ipt_score, dim=0, keepdim=False).view(-1, 1)
                name_mat = n.replace("lora_B_ir", "%s")
                if name_mat not in combine_dict: 
                    combine_dict[name_mat] = [comb_ipt]
                else:
                    combine_dict[name_mat].append(comb_ipt)
...
class BaseTrainer_m:
    ...
    def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None):
        ...
        self.trainset_rgb, self.testset_rgb, self.trainset_ir, self.testset_ir = self.get_dataset_m(self.data)
        self.ema = None
    ...
    def _setup_train(self, world_size):
        ...
        self.train_loader = self.get_dataloader(self.trainset_rgb, self.trainset_ir, batch_size=batch_size, rank=RANK, mode="train")
        if RANK in (-1, 0):
            # Note: When training DOTA dataset, double batch size could get OOM on images with >2000 objects.
            self.test_loader = self.get_dataloader(
                self.testset_rgb, self.testset_ir, batch_size=batch_size if self.args.task == "obb" else batch_size * 2, rank=-1, mode="val"
            )
            ...
            for i, batch in pbar:
                ...
                if RANK in (-1, 0):
                    pbar.set_description(
                        ...
                        % (f"{epoch + 1}/{self.epochs}", mem, *losses, batch["cls"].shape[0], batch["img_rgb"].shape[-1])
                    )
    ...
    @staticmethod
    def get_dataset_m(data):
        """
        Get train, val path from data dict if it exists.

        Returns None if data format is not recognized.
        """
        return data["train_rgb"], data.get("val_rgb") or data.get("test_rgb"), \
        data["train_ir"], data.get("val_ir") or data.get("test_ir")
