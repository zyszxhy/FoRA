...
def get_flops(model, imgsz=640):
    ...
    try:
        ...
        try:
            ...
            if type(model).__name__.endswith('_m'):
                flops = thop.profile(deepcopy(model), inputs=[im, im], verbose=False)[0] / 1e9 * 2
            else:
            ...
            if type(model).__name__.endswith('_m'):
                return thop.profile(deepcopy(model), inputs=[im, im], verbose=False)[0] / 1e9 * 2
