...
def plot_images_m(
    ...
    paths_rgb=None,
    paths_ir=None,
    fname_rgb="images_rgb.jpg",
    fname_ir="images_ir.jpg",
    names=None,
    on_plot=None,
    max_subplots=16,
    save=True,
    ...
    """Plot image grid with labels."""
    if isinstance(images_rgb, torch.Tensor):
        images_rgb = images_rgb.cpu().float().numpy()
    if isinstance(images_ir, torch.Tensor):
        images_ir = images_ir.cpu().float().numpy()
    if isinstance(cls, torch.Tensor):
        cls = cls.cpu().numpy()
    if isinstance(bboxes, torch.Tensor):
        bboxes = bboxes.cpu().numpy()
    ...
    max_size = 1920  # max image size
    bs, _, h, w = images_rgb.shape  # batch size, _, height, width
    bs = min(bs, max_subplots)  # limit plot images
    ns = np.ceil(bs**0.5)  # number of subplots (square)
    if np.max(images_rgb[0]) <= 1:
        images_rgb *= 255  # de-normalise (optional)
    if np.max(images_ir[0]) <= 1:
        images_ir *= 255  # de-normalise (optional)

    # Build Image
    mosaic_rgb = np.full((int(ns * h), int(ns * w), 3), 255, dtype=np.uint8)  # init
    mosaic_ir = np.full((int(ns * h), int(ns * w), 3), 255, dtype=np.uint8)  # init
    for i in range(bs):
        x, y = int(w * (i // ns)), int(h * (i % ns))  # block origin
        mosaic_rgb[y : y + h, x : x + w, :] = images_rgb[i].transpose(1, 2, 0)
        mosaic_ir[y : y + h, x : x + w, :] = images_ir[i].transpose(1, 2, 0)
    ...
    if scale < 1:
        h = math.ceil(scale * h)
        w = math.ceil(scale * w)
        mosaic_rgb = cv2.resize(mosaic_rgb, tuple(int(x * ns) for x in (w, h)))
        mosaic_ir = cv2.resize(mosaic_ir, tuple(int(x * ns) for x in (w, h)))

    # Annotate
    fs = int((h + w) * ns * 0.01)  # font size
    annotator_rgb = Annotator(mosaic_rgb, line_width=round(fs / 10), font_size=fs, pil=True, example=names)
    annotator_ir = Annotator(mosaic_ir, line_width=round(fs / 10), font_size=fs, pil=True, example=names)
    for i in range(bs):
        x, y = int(w * (i // ns)), int(h * (i % ns))  # block origin
        annotator_rgb.rectangle([x, y, x + w, y + h], None, (255, 255, 255), width=2)  # borders
        annotator_ir.rectangle([x, y, x + w, y + h], None, (255, 255, 255), width=2)  # borders
        if paths_rgb:
            annotator_rgb.text((x + 5, y + 5), text=Path(paths_rgb[i]).name[:40], txt_color=(220, 220, 220))  # filenames
        if paths_ir:
            annotator_ir.text((x + 5, y + 5), text=Path(paths_rgb[i]).name[:40], txt_color=(220, 220, 220))
        if len(cls) > 0:
            idx = batch_idx == i
            classes = cls[idx].astype("int")
            labels = confs is None
            ...
            if len(bboxes):
                ...
                for j, box in enumerate(boxes.astype(np.int64).tolist()):
                    ...
                    if labels or conf[j] > conf_thres:
                        label = f"{c}" if labels else f"{c} {conf[j]:.1f}"
                        annotator_rgb.box_label(box, label, color=color, rotated=is_obb)
                        annotator_ir.box_label(box, label, color=color, rotated=is_obb)

            elif len(classes):
                for c in classes:
                    color = colors(c)
                    c = names.get(c, c) if names else c
                    annotator_rgb.text((x, y), f"{c}", txt_color=color, box_style=True)
                    annotator_ir.text((x, y), f"{c}", txt_color=color, box_style=True)
            ...
            if len(kpts):
                ...
                for j in range(len(kpts_)):
                    if labels or conf[j] > conf_thres:
                        annotator_rgb.kpts(kpts_[j])
                        annotator_ir.kpts(kpts_[j])
            ...
            if len(masks):
                ...
                im_rgb = np.asarray(annotator_rgb.im).copy()
                im_ir = np.asarray(annotator_ir.im).copy()
                for j in range(len(image_masks)):
                    if labels or conf[j] > conf_thres:
                        color = colors(classes[j])
                        mh, mw = image_masks[j].shape
                        if mh != h or mw != w:
                            ...
                            mask = image_masks[j].astype(bool)
                        with contextlib.suppress(Exception):
                            im_rgb[y : y + h, x : x + w, :][mask] = (
                                im_rgb[y : y + h, x : x + w, :][mask] * 0.4 + np.array(color) * 0.6
                            )
                            im_ir[y : y + h, x : x + w, :][mask] = (
                                im_ir[y : y + h, x : x + w, :][mask] * 0.4 + np.array(color) * 0.6
                            )
                annotator_rgb.fromarray(im_rgb)
                annotator_ir.fromarray(im_ir)
    if not save:
        return np.asarray(annotator_rgb.im), np.asarray(annotator_ir.im)
    annotator_rgb.im.save(fname_rgb)  # save
    annotator_ir.im.save(fname_ir)  # save
    if on_plot:
        on_plot(fname_rgb)
        on_plot(fname_ir)
